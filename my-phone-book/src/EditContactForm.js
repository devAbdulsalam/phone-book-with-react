...
// EdithContactFromLocalStorage

useEffect (() =>  {
  setMyContact(() => contactList.filter((contact) => {
  if(contact.id === userId){
     return setMyContact(contact)
    }
     return console.log("here is the currentContact")
  }))
},[contactList, userId])

const [mycontact, setMyContact] = useState([])
const [contact, setContact] = useState({firstName: mycontact.firstName})
// console.log(contact.firstName : mycontact.firstName)
            ...
            <div className="relative w-full">
                <div id="contactHead" className="bg-gray-600 p-2 top-0 z-10 w-full">
                    <div className="flex justify-between items-center w-full px-3">
                        <div className="text-white text-2xl" onClick={() => navigate(`/contact/${id}`)}>
                            <i className="fa fas fa-times cursor-pointer"></i>
                        </div>
                        <div className="">
                            <h1 className="text-white text-xl font-bold">Edit contact</h1>
                        </div>
                    </div>
                </div >
