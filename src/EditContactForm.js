...
const EditContactForm = () => {
    ...
    useEffect (() =>  {
        setMyContact(() => contactList.filter((contact) => contact.id === userId))
    },[contactList, userId])

    mycontact.map((contact) =>{
        const { id, firstName, lastName, phone, email, relationship, address, company} = contact 
        // setContact({ ...contact, id:id, firstName: firstName, lastName: lastName, phone: phone, email: email, company: company, relationship: relationship, address: address })
